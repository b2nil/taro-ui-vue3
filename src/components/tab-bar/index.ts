...
const AtTabBar = defineComponent({
  ...
  setup(props: AtTabBarProps, { slots }) {
    ...
    return () => {
      ...
      const tabItemClass = computed(() => (i) => classNames(
        'at-tab-bar__item', {
        'at-tab-bar__item--active': props.current === i
      }))

      const tabBarItemIconClass = computed(() => (item, i) => classNames(
      ...
      const tabBarItemInnerImgClass = computed(() => (selected: boolean) => classNames(
      ...
      return (
        h(View, {
          ...
          default: () =>
            props.tabList.map((item: TabItem, i: number) => (
              // tab-bar item
              h(View, {
                key: item.title,
