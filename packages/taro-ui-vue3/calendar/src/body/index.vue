<template>
    ...
    <view class="main__body body">
      <view class="body__slider body__slider--now">
        <at-calendar-date-list
          ...
          @click="(item) => $emit('day-click', item)"
          @long-click="(item) => $emit('long-click', item)"
        />
      </view>
    </view>
  </view>
  ...
  >
    ...
    <swiper
      ...
      v-bind="{ [`onAnimation${isAlipay ? 'End' : 'Finish'}`]: handleAnimateFinish }"
    >
      <swiper-item
...
export default defineComponent({
  ...
  setup(props: AtCalendarBodyProps, { emit }) {
    ...
    const h5MainBodyClasses = computed(() => ({
